...
const AddStaff = ({ onAddOrUpdateStaff, editStaff, setEditStaff }) => {
  ...
  useEffect(() => {
    const fetchNGOList = async () => {
      try {
        const reqType = "list";
        const ngos = await retrieveNGOList(reqType); // Correct format
        console.log("NGO List API Response:", ngos); // Debugging
        setNgoList(Array.isArray(ngos.payload) ? ngos.payload : []); // Use ngos.payload
      } catch (error) {
        console.error("Error fetching NGO list:", error);
        setNgoList([]); // Prevent crash
      }
    };
  ...
  return (
    <div className="bg-white p-8 shadow-md rounded-lg">
      ...
      >
        ...
        {/* NGO Name Dropdown (Only for userRole === 1) */}
        {userRole === 1 && (
          <div>
